...
- The service has a Docs tab: what a spec can contain, form by form, with
  eleven complete examples that load into the editor and run. Written for
  people using tilegen, not people reading its source; a test generates
  every documented example, and another keeps the language user-facing.
